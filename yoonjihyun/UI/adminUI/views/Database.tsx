...
const Database: React.FC<DatabaseProps> = ({ onRowClick }) => {
  const [filter, setFilter] = useState('All');

  const filteredData = filter === 'All' 
    ? MOCK_HAZARDS 
    : MOCK_HAZARDS.filter(h => h.status === filter);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 mt-1">시스템에 등록된 모든 위험 데이터를 조회하고 관리합니다.</p>
        </div>
      ...
      <div className="flex justify-center mt-6">
        <nav className="flex gap-2">
            ...
            <button className="w-10 h-10 flex items-center justify-center rounded-lg border border-transparent text-slate-500 hover:bg-slate-100">2</button>
            <button className="w-10 h-10 flex items-center justify-center rounded-lg border border-transparent text-slate-500 hover:bg-slate-100">3</button>
        </nav>
      </div>
    </div>
  );
