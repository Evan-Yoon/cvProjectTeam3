...
const Reports: React.FC<ReportsProps> = ({ type, onRowClick }) => {
  const [searchTerm, setSearchTerm] = useState('');
  
  const filteredData = MOCK_HAZARDS.filter(item => 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            {type === 'B2B' ? '기업 고객 (Corporate) 관련 신고 내역입니다.' : '정부/지자체 (Government) 관련 신고 내역입니다.'}
          </p>
        </div>
